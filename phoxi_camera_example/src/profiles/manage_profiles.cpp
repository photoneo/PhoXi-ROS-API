...
typename SrvT::Response::SharedPtr call_service(
        std::shared_ptr<rclcpp::Node> node, const std::string& service_name, typename SrvT::Request::SharedPtr request = std::make_shared<typename SrvT::Request>()) {
...
bool check(std::shared_ptr<rclcpp::Node> node, const std::string& action, const std::string& name, bool success, const std::string& message) {
    if (!success) {
        RCLCPP_ERROR(node->get_logger(), "%s '%s' FAILED: %s", action.c_str(), name.c_str(), message.c_str());
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    auto create_resp = call_service<phoxi_camera_msgs::srv::CreateProfile>(node, "/phoxi_camera/profiles/create", create_req);
    if (!create_resp || !check(node, "Create", profile_name, create_resp->success, create_resp->message)) {
        rclcpp::shutdown();
        return 1;
    }
    ...
    auto activate_resp = call_service<phoxi_camera_msgs::srv::SetActiveProfile>(node, "/phoxi_camera/profiles/set_active", activate_req);
    if (activate_resp) {
    ...
    auto update_resp = call_service<phoxi_camera_msgs::srv::UpdateProfile>(node, "/phoxi_camera/profiles/update", update_req);
    if (update_resp) {
    ...
    auto delete_resp = call_service<phoxi_camera_msgs::srv::DeleteProfile>(node, "/phoxi_camera/profiles/delete", delete_req);
    if (delete_resp) {
