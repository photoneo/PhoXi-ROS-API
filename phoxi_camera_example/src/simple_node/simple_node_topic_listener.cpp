...
void color_camera_image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
    RCLCPP_INFO(rclcpp::get_logger("topic_listener_node"),
                "SUCCESS: Received a ColorCameraImage message %ux%u encoding=%s.",
...
int main(int argc, char* argv[]) {
    ...
    auto color_camera_image_sub = node->create_subscription<sensor_msgs::msg::Image>(
        "/color_camera_image", 10, color_camera_image_callback);

    RCLCPP_INFO(node->get_logger(),
                "Listening for messages on /point_cloud and /color_camera_image...");
